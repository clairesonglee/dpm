...
int DARSHAN_DECL(ncmpi_delete)(const char *filename, MPI_Info info)
{
    int ret;
    char* tmp;
    double tm1, tm2;

    MAP_OR_FAIL(ncmpi_delete);

    tm1 = darshan_core_wtime();
    ret = __real_ncmpi_delete(filename, info);
    tm2 = darshan_core_wtime();
    if(ret == 0)
    {
        PNETCDF_PRE_RECORD();
        PNETCDF_RECORD_DELETE(filename, tm1, tm2);
        PNETCDF_POST_RECORD();
    }
