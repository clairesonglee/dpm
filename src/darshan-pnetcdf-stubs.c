...
const char * ncmpi_strerror(int ncerr) __attribute__((weak));
...
const char * ncmpi_strerror(int ncerr) 
{
    ...
    if(rank == 0)
    {
        fprintf(stderr, "WARNING: Darshan ncmpi_strerror() stub called; this is probably the result of a link-time problem.\n");
    }
...
int ncmpi_delete(const char *filename, MPI_Info info) __attribute__((weak));
...
int ncmpi_delete(const char *filename, MPI_Info info)
{
    ...
    if(rank == 0)
    {
        fprintf(stderr, "WARNING: Darshan ncmpi_delete() stub called; this is probably the result of a link-time problem.\n");
    }
